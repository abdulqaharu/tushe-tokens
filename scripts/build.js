...
function buildThemeMap(tokens) {
  ...
  for (const dottedPath of tokens.keys()) {
    ...
    // Always a var() reference, never a copied literal. tokens.css already
    // defines this variable with the correct default and cascade behavior
    // (including mode-switchable ones like font.family.sans), so theme-map
    // just points at it rather than re-deriving anything.
