...
function buildCss(tokens) {
  ...
  for (const [dottedPath, token] of tokens.entries()) {
    // text-styles values are structured objects (fontSize/lineHeight/etc),
    // a third shape this generic walker doesn't understand, literal or
    // mode-map are the only two it knows. Handled entirely by
    // buildTextStyles instead, skipped here rather than guessed at.
    if (dottedPath.startsWith("text-styles.")) continue;
...
// --- 5. text-styles.css: compound Tailwind text-* utilities from Figma text styles ---
//
// Mobile-first: mobile values are the unconditional default, web values
// override at the MOBILE_BREAKPOINT below where they actually differ. This
// mirrors Tailwind's own mobile-first convention (unprefixed utilities
// apply everywhere, breakpoint-prefixed ones override at larger sizes),
// implemented here as a plain CSS custom-property override under a media
// query, the same mechanism already used for [data-theme="Dark"] etc,
// just a breakpoint condition instead of an attribute selector.
//
// font-family is deliberately excluded from every one of these, per this
// session's decision: baking a literal font-family into 75 separate
// compound utilities would silently stop responding to data-font
// switching. Pair these with font-sans (inherited from the root layout
// already) or font-mono for code-* specifically.
const MOBILE_BREAKPOINT = "768px";

function collectTextStyles(tokens, platform) {
  const prefix = `text-styles.${platform}.`;
  const result = new Map();
  for (const [dottedPath, token] of tokens.entries()) {
    if (!dottedPath.startsWith(prefix)) continue;
    const flatKey = dottedPath.slice(prefix.length).split(".").join("-");
    result.set(flatKey, token.value);
  }
  return result;
}

function textStyleDeclarations(key, style) {
  const letterSpacingEm = style.letterSpacing !== null ? style.letterSpacing / style.fontSize : 0;
  return [
    `  --text-${key}: ${style.fontSize}px;`,
    `  --text-${key}--line-height: ${style.lineHeight !== null ? style.lineHeight + "px" : "normal"};`,
    `  --text-${key}--letter-spacing: ${letterSpacingEm}em;`,
    `  --text-${key}--font-weight: ${style.fontWeight};`,
  ];
}

function buildTextStyles(tokens) {
  const mobileStyles = collectTextStyles(tokens, "mobile");
  const webStyles = collectTextStyles(tokens, "web");
  const allKeys = new Set([...mobileStyles.keys(), ...webStyles.keys()]);

  const defaultLines = [];
  const overrideLines = [];

  for (const key of allKeys) {
    const mobile = mobileStyles.get(key);
    const web = webStyles.get(key);
    // Mobile-first default. Falls back to web when a key has no mobile
    // equivalent at all (currently only web/code/*, code blocks aren't
    // given a distinct mobile size), in which case that's the only value
    // that exists, unconditional, no override needed.
    const primary = mobile || web;
    defaultLines.push(...textStyleDeclarations(key, primary));

    if (mobile && web && JSON.stringify(mobile) !== JSON.stringify(web)) {
      overrideLines.push(...textStyleDeclarations(key, web));
    }
  }

  let css =
    `/* Generated by scripts/build.js from src/tokens.json. Do not hand-edit. */\n` +
    `/* Mobile-first: default values are the mobile text styles, overridden at ${MOBILE_BREAKPOINT}+ with web values where they differ. font-family excluded on purpose, pair with font-sans (inherited from the root layout) or font-mono for code-*. */\n` +
    `/* Plain @theme, not @theme inline: these values are literal numbers, not references to another variable. Inlining them would bake the number directly into the generated utility, exactly the mistake that broke the media-query override below the first time this was written. */\n\n`;
  css += `@theme {\n${defaultLines.join("\n")}\n}\n`;
...
const TEXT_STYLES_OUT = path.join(__dirname, "..", "dist", "text-styles.css");
const textStylesCss = buildTextStyles(tokens);
fs.writeFileSync(TEXT_STYLES_OUT, textStylesCss);
console.log(`Wrote ${TEXT_STYLES_OUT} (${textStylesCss.length} bytes)`);